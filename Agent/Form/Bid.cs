...
namespace Agent.Form
{
    ...
    {    datebase db = new datebase();
        ...
        public void Objectpolicyholder_load2()
        {
            sqlConnection.Open();
            DataSet dataSet = new DataSet();
            SqlDataAdapter command = new SqlDataAdapter($@"Select idbid as [Номер заявки], 
            date as [Дата заявки], bid.idvida, vid.name as [Вид страхования],
            status as Статус,bid.idpolicyholder, 
            (policyholder.firdtname +' '+policyholder.name+' '+policyholder.lastname) as Страхователь,
            bid.note as Пожелания from bid inner join vid on bid.idvida=vid.idvida inner join 
            policyholder on bid.idpolicyholder=policyholder.idpolicyholder 
            where bid.idpolicyholder={policyholder.idakk}", sqlConnection);
            command.Fill(dataSet);
            dataGridView1.DataSource = dataSet.Tables[0].DefaultView;
            sqlConnection.Close();
            dataGridView1.Columns[2].Visible = false;
                        ...
                        //    dateTimePicker1.Value = Convert.ToDateTime(dataGridView1.CurrentRow.Cells[1].Value.ToString());
                            ...
                            for (int i = 0; i < dataGridView2.RowCount; i++)
                            {
                                ...
                                if (dataGridView1.CurrentRow.Cells[2].Value.ToString() == dataGridView2[0, i].Value.ToString())
                                {
                                    ...
                                    id2 =Convert.ToInt32( dataGridView2[0, i].Value.ToString());
                                    break;
                                }
                            }
            ...
            try
            {
                if (label6.Text == "Добавить заявку")
                {
                    if (comboBox1.SelectedIndex != -1&&id2!=0)
                    {
                            ...
                            sqlConnection.Open();
                            SqlCommand command = new SqlCommand($@"INSERT INTO [bid](date,idvida,status,idpolicyholder,note)
                            VALUES (@d,@i,@s,@ip,@n);", sqlConnection);
...
and 
((vid.name NOT Like '%гражданской%' and vid.name NOT Like '%пассажиров%' and vid.name NOT Like '%железнодорожного%' and vid.name NOT Like '%профессиональной%'
and position.name NOT Like '%начальник%' ) 
or (position.name  Like '%начальник%' ))", sqlConnection);
                command.Fill(dataSet);
