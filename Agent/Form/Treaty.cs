    ...
    public partial class Treaty : UserControl
    {
        ...
        public void Tread_load()
        {
            if (insurerak != null)
            {
                try
                {
                  vop = $@"where insurer.idinsurer={insurerak.idakk}";
                }
                catch { }
            }else
            if (policyholder!= null)
            {
                try
                {
                   vop = $@"where bid.idpolicyholder={policyholder.idakk}";
                }
                catch { }
            }
            else { vop = ""; }

            string zap = $@"Select Distinct treaty.idtreaty as [Номер договора],treaty.idinsurer,(insurer.firstname+' '+insurer.name+' '+insurer.lastname) as [Страховщик],
bid.idpolicyholder,
(policyholder.firdtname+' '+policyholder.name+' '+policyholder.lastname) as Страхователь,
bid.idvida,vid.name as [Вид страхования],
...
suminsured as [Страховая сумма, BYN], 
...
THEN 0.2 else (CASE WHEN(vid.name='Страхование средств железнодорожного транспорта')
THEN 24 else 1 end) end) end) end) end) end) end))
...
THEN 1 else (CASE WHEN(vid.name='Страхование гражданской ответственности за причинение вреда в связи с осуществлением профессиональной деятельности')
THEN 0.2 else (CASE WHEN(vid.name='Страхование средств железнодорожного транспорта')
THEN 24 else 1 end) end) end) end) end) end) end)) else null end) end) end))),2)as decimal(18,2)) )
...
else (CASE WHEN(vid.name='Страхование средств железнодорожного транспорта')
THEN Cast(ROUND((0.24),2)as decimal(18,2))  
else 1 end) end) end) end) end) end) end)as [Базовый коэффициент, %],
(select Distinct Cast(ROUND((((CASE WHEN(treaty.term='Единовременно' and vid.name!='Страхование средств железнодорожного транспорта' )
THEN  ((treaty.vznos/treaty.suminsured)*100)
else (CASE WHEN(treaty.term='В два срока' and vid.name!='Страхование средств железнодорожного транспорта' ) THEN  ((treaty.vznos*2)/treaty.suminsured)*100
else (CASE WHEN(treaty.term='Ежеквартально' and vid.name!='Страхование средств железнодорожного транспорта' )
THEN  ((treaty.vznos*4)/treaty.suminsured)*100
else (select Distinct Cast(ROUND((((CASE WHEN(treaty.term='Единовременно' and vid.name='Страхование средств железнодорожного транспорта' )
THEN  ((treaty.vznos/treaty.suminsured))
else (CASE WHEN(treaty.term='В два срока' and vid.name='Страхование средств железнодорожного транспорта' ) THEN  ((treaty.vznos*2)/treaty.suminsured)
else (CASE WHEN(treaty.term='Ежеквартально' and vid.name='Страхование средств железнодорожного транспорта' )
THEN  ((treaty.vznos*4)/treaty.suminsured)
else null end) end) end))),2)as decimal(18,2)) ) end) end) end))),2)as decimal(18,2)) )
as [Страховой тариф, %],
...
else null end) end) end)))as [Страховая премия, BYN],
dateconclusion as [Дата первого взноса],vznos as [Первый взнос, BYN],
...
THEN  vznos else null end)as [Второй взноса, BYN],
...
)as [Третий взноса, BYN],(CASE WHEN(treaty.term='Ежеквартально')
...
else null end)as [Четвертый взноса, BYN],treaty.idbid
from
...
bid.idpolicyholder,bid.idvida,vid.name,treaty.vznos,treaty.idbid,
(policyholder.firdtname+' '+policyholder.name+' '+policyholder.lastname),
dateconclusion , term , 
datestart , datefinish ,suminsured ,pay.vidpay,
correctionfactor.coefficient,pay.datepay";
...
where correctionfactor.name='Страховая сумма' and vid.idvida={id3}";
                ...
                for (int i = 0; i < kk; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //Фрагмент кода расчета страховой премии
                            string query3 = $@"Select coefficient,correctionfactor.note,idcorrectionfactor 
                            from correctionfactor inner join vid on vid.idvida=correctionfactor.idvida 
                            where correctionfactor.name='Страховая сумма' and vid.idvida={id3} order by 
                            correctionfactor.note ASC";
        ...
        private void button11_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (comboBox6.SelectedIndex != -1 && comboBox1.SelectedIndex != -1 && comboBox2.SelectedIndex != -1 && comboBox5.SelectedIndex != -1)
            {
                sqlConnection.Open();
...
inner join bid on bid.idbid=treaty.idbid where treaty.idbid={dataGridView1.CurrentRow.Cells[24].Value.ToString()}";
        ...
        public void two()
        {
            ...
            string zapros = $@"Select idsostav,sostav.idtreaty, sostav.idtrain, train.name as [Название транспорта],train.nomer as [Номер транспорта], 
price as [Страховая стоимость], summa as [Страховая сумма],
cast( coefficient*0.24*{kofi} as decimal(18,2)) 
as [Страховой тариф],cast( summa*(coefficient*0.24*{kofi}) as decimal(18,2))  as [Страховая премия]  
from train,sostav,correctionfactor
where sostav.idtreaty is null
and train.idtrain=sostav.idtrain and  correctionfactor.name like '%средств%'
and coefficient in (
select TOP(Select count(sostav.idsostav) from sostav left join treaty on sostav.idtreaty=treaty.idtreaty where sostav.idtreaty is null) 
(
(case when(
(case when (SUBSTRING(note,0,3)='>=' or SUBSTRING(note,0,3)='<=')then SUBSTRING(note,0,3) 
else SUBSTRING(note,0,2) end)='>=') then 
(case when(
(case when (SUBSTRING(note,0,3)='>=' or SUBSTRING(note,0,3)='<=')then SUBSTRING(note,3,len(note)) 
else SUBSTRING(note,2,len(note)) end)<= (Select count(sostav.idsostav) from sostav left join treaty on sostav.idtreaty=treaty.idtreaty where sostav.idtreaty is null))
then coefficient else 
0
end
)
...
and train.idtrain=sostav.idtrain and  correctionfactor.name like '%средств%'
group by idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer, 
price, summa ,sostav.idsostav,note,coefficient
order by correctionfactor.note Desc
)
group by idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer, 
price, summa ,sostav.idsostav,note,coefficient
...
union
Select idsostav,sostav.idtreaty, sostav.idtrain, train.name as [Название транспорта],train.nomer as [Номер транспорта], 
price as [Страховая стоимость], summa as [Страховая сумма],
cast( coefficient*0.24*{kofi} as decimal(18,2)) 
as [Страховой тариф],cast( summa*(coefficient*0.24*{kofi}) as decimal(18,2))  as [Страховая премия]  
from train,sostav,correctionfactor
...
union
Select Distinct idsostav,sostav.idtreaty, sostav.idtrain, train.name as [Название транспорта],train.nomer as [Номер транспорта], 
price as [Страховая стоимость], summa as [Страховая сумма],
cast( 1*0.24*{kofi} as decimal(18,2)) 
as [Страховой тариф],cast( summa*(1*0.24*{kofi}) as decimal(18,2))  as [Страховая премия]  
from train,sostav,correctionfactor
where sostav.idtreaty is null
and train.idtrain=sostav.idtrain and  correctionfactor.name like '%средств%'
group by idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer, 
...
order by correctionfactor.note Desc)=1";
            sqlConnection.Open();
                DataSet dataSet = new DataSet();
                SqlDataAdapter command = new SqlDataAdapter($@"{zapros}", sqlConnection);
                command.Fill(dataSet);
                dataGridView2.DataSource = dataSet.Tables[0].DefaultView;
                sqlConnection.Close();
            ...
            if (dataGridView2.RowCount > 0)
            {
                ...
                try
                {
                    if (dataGridView2.Rows.Count > 0)
                    {
                        ...
                        {
                            sumid = sumid + Convert.ToDecimal(dataGridView2[6, i].Value.ToString());
                            textBox13.Text = Convert.ToString(Convert.ToDecimal(textBox13.Text) + Convert.ToDecimal(dataGridView2[8, i].Value.ToString()));
                        }
                    }
                }
                catch { textBox13.Text = "0"; }
            }
        }
        decimal sumid = 0;
        ...
        {count = 0;
            ...
            id3 = Convert.ToInt32(dataGridView3.CurrentRow.Cells[1].Value.ToString());
                ...
                for (int i = 0; i < dataGridView1.RowCount; i++)
                {
                    if (Convert.ToDateTime(dataGridView1[10, i].Value.ToString()) <= DateTime.Today && Convert.ToInt32(comboBox2.SelectedValue) == Convert.ToInt32(dataGridView1[3, i].Value.ToString()))
                    { count++; }
                }
            }
            catch { }
            if (count == 0)
            {
                ...
                if (dataGridView3.CurrentRow.Cells[2].Value.ToString().ToLower().Contains("железнодорожного транспорта"))
                {
                    x = 1;
                    pod = false;
                    cof3 = Convert.ToDecimal("0,24");
                }
                else
                {
                    cof3 = Convert.ToDecimal("1,00");
...
or (4-count(pay.idtreaty)!=0 and treaty.term='Ежеквартально') ";
        ...
        {
            kol = 0;
            if (textBox12.Text != "" && textBox3.Text != "" && idt != 0)
            {
                for (int i = 0; i < dataGridView2.RowCount; i++)
                {
                    if (idt == Convert.ToInt32(dataGridView2[2, i].Value.ToString()))
                    {
                        kol++;
                    }
                }
                if (kol == 0)
                {
                    if (Convert.ToDecimal(textBox3.Text) > Convert.ToDecimal(textBox12.Text))
                    {
                        sqlConnection.Open();
                        SqlCommand command = new SqlCommand($@"INSERT INTO [sostav](idtrain,price,summa) VALUES (@t,@p,@s);", sqlConnection);
                        command.Parameters.AddWithValue("@t", (dataGridView1.CurrentRow.Cells[0].Value.ToString()));
                        command.Parameters.AddWithValue("@p", Convert.ToDecimal(textBox3.Text));
                        command.Parameters.AddWithValue("@s", Convert.ToDecimal(textBox12.Text));
                        command.ExecuteNonQuery();
                        sqlConnection.Close();
                        two();
                        clear2();
                    }
                    else { MessageBox.Show("Страховая сумма не может быть больше страховой стоимости!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error); }
                }
                else { MessageBox.Show("Транспорт уже используется!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            }
            else { MessageBox.Show("Заполните поля!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        int j = 0;
            ...
            if (id4 != 0)
            {
                if (textBox12.Text != "" && textBox3.Text != "" && idt != 0)
                {
                    for (int i = 0; i < dataGridView2.RowCount; i++)
                    {
                        ...
                        {
                            kol++;
                            j = Convert.ToInt32(dataGridView2.CurrentRow.Cells[2].Value.ToString());
                        }
                    }
                    if (kol == 0 || j == idt)
                    {
                        ...
                        {
                            sqlConnection.Open();
                            SqlCommand command = new SqlCommand($@"UPDATE sostav SET idtrain=@t,price=@p,summa=@s WHERE idsostav=@id", sqlConnection);
                            command.Parameters.AddWithValue("@t", (idt));
                            command.Parameters.AddWithValue("@p", Convert.ToDecimal(textBox3.Text));
                            command.Parameters.AddWithValue("@s", Convert.ToDecimal(textBox12.Text));
                            command.Parameters.AddWithValue("@id", (id4));
                            command.ExecuteNonQuery();
                            sqlConnection.Close();
                            two();
                            clear2();
                            button14.Enabled = true; button13.Enabled = true;
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (dataGridView2.RowCount > 0)
                {
                    id4 = Convert.ToInt32(dataGridView2.CurrentRow.Cells[0].Value.ToString());
                }
            }
            catch { }
        }
        ...
        private void dataGridView2_DoubleClick(object sender, EventArgs e)
        {
            if (id4 != 0)
            {
                button14.Enabled = false; button13.Enabled = false;
                dataGridView2.Enabled = false;
                clear2();
                idt = 0;
                textBox3.Text = dataGridView2.CurrentRow.Cells[5].Value.ToString();
                textBox12.Text = dataGridView2.CurrentRow.Cells[6].Value.ToString();
                for (int i = 0; i < dataGridView1.RowCount; i++)
                {
                    if (dataGridView1[3, i].Value.ToString() == dataGridView2.CurrentRow.Cells[4].Value.ToString())
                    {
                        idt = Convert.ToInt32(dataGridView1[0, i].Value.ToString());
                        dataGridView1.Rows[i].Selected = true;
                        dataGridView1.Rows[i].DefaultCellStyle.SelectionForeColor = Color.Black;
                        dataGridView1.Rows[i].DefaultCellStyle.SelectionBackColor = Color.FromArgb(212, 236, 252);
                    }
            ...
            else { MessageBox.Show("Выберите строку для редактирования!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        private void button14_Click(object sender, EventArgs e)
        {
            if (id4 != 0)
            {
                try
                {
                    if (MessageBox.Show($@"Вы уверены что хотите удалить транспорт {dataGridView2.CurrentRow.Cells[3].Value.ToString()}?", "Удаление", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        sqlConnection.Open();
                        string query = $@"DELETE FROM [sostav] WHERE [idsostav] = 
        ...
        public void seven()
        {
            if (textBox7.Text != "" || textBox13.Text != "")
            {
                ...
                else if (comboBox6.SelectedIndex == 1)
                {
                    ...
                    {
                        textBox8.Text = Convert.ToString(Math.Round((Convert.ToDecimal(textBox13.Text) / 2), 2));
                        textBox9.Text = textBox8.Text;
                        textBox10.Text = "";
                        textBox11.Text = "";
                    }
                ...
                else if (comboBox6.SelectedIndex == 2)
                {
                    ...
                    {
                        textBox8.Text = Convert.ToString(Math.Round((Convert.ToDecimal(textBox13.Text) / 4), 2));
                        textBox9.Text = textBox8.Text;
                        textBox10.Text = textBox8.Text;
                        textBox11.Text = textBox8.Text;
                    }
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (yes == true)
            {
                if (policyholder != null)
                {
                    policyholder.Height = 866;
                    policyholder.panel6.Height = 823;
                }
                x = 0;
                panel11.Visible = false;
                panel7.Visible = false;
                ...
                clear();
                clear2();
                string query = "select Distinct policyholder.idpolicyholder,(firdtname+' '+name+' '+lastname) as p from policyholder ";
                SqlDataAdapter sqlDbDataAdapter = new SqlDataAdapter(query, sqlConnection);
                DataSet dataSet = new DataSet();
                sqlDbDataAdapter.Fill(dataSet);
                comboBox2.DataSource = dataSet.Tables[0];
                ...
                dateTimePicker3.Value = Convert.ToDateTime(dataGridView1.CurrentRow.Cells[10].Value.ToString());
                comboBox6.SelectedItem = dataGridView1.CurrentRow.Cells[8].Value.ToString();
                comboBox6.Enabled = false;
                three();
                comboBox2.SelectedValue = dataGridView1.CurrentRow.Cells[3].Value.ToString();
                comboBox2.Enabled = false;
                string query3 = $@"Select vidpay from treaty inner join pay on pay.idtreaty= treaty.idtreaty where pay.idtreaty={Convert.ToInt32(dataGridView1.CurrentRow.Cells[0].Value.ToString())}";
                System.Data.DataTable data3 = new System.Data.DataTable();
                SqlDataAdapter command3 = new SqlDataAdapter(query3, sqlConnection);
                command3.Fill(data3);
                DataColumn column2 = data3.Columns[0];
                DataRow row2 = data3.Rows[0];
                ...
                try
                {
                    ...
                    SqlDataAdapter command = new SqlDataAdapter($@"select bid.idbid,vid.idvida, vid.name as Название, bid.note as Пожелания from vid, policyholder,bid " +
                    $@"where bid.idvida=vid.idvida and bid.idpolicyholder=policyholder.idpolicyholder and bid.idbid={Convert.ToInt32(dataGridView1.CurrentRow.Cells[24].Value.ToString())}", sqlConnection);
                    command.Fill(dataSet2);
                    dataGridView3.DataSource = dataSet2.Tables[0].DefaultView;
                    sqlConnection.Close();
                    dataGridView3.Columns[0].Visible = false;
                    dataGridView3.Columns[1].Visible = false;
                    dataGridView3.AllowUserToAddRows = false;
                ...
                if (dataGridView3[2, 0].Value.ToString() == "Медицинское страхование")
                {
                    cof3 = Convert.ToDecimal("0,52");
                }
                else if (dataGridView3[2, 0].Value.ToString() == "Страхование от несчатного случая")
                {
                    cof3 = Convert.ToDecimal("1,10");
                }
                else if (dataGridView3[2, 0].Value.ToString() == "Обязательное страхование гражданской ответственности перевозчика перед пассажирами")
                {
                    cof3 = Convert.ToDecimal("1,00");
                }
                else if (dataGridView3[2, 0].Value.ToString() == "Страхование от несчастных случаев пассажиров железного транспорта")
                {
                    cof3 = Convert.ToDecimal("0,50");
                }
                else if (dataGridView3[2, 0].Value.ToString() == "Страхование жизни")
                {
                    cof3 = Convert.ToDecimal("1,00");
                }
                else if (dataGridView3[2, 0].Value.ToString() == "Страхование гражданской ответственности за причинение вреда в связи с осуществлением профессиональной деятельности")
                {
                    cof3 = Convert.ToDecimal("0,20");
                }
                if (dataGridView3[2, 0].Value.ToString().ToLower().Contains("железнодорожного транспорта"))
                {
                    x = 1;
                    cof3 = Convert.ToDecimal("0,24");
                }
                ...
                try
                {
                    ...
                    SqlDataAdapter command = new SqlDataAdapter($@"Select train.name + '; ' + cast(train.nomer as varchar(50)) as [Название транспорта, номер],
                    train.type + '; ' + cast(train.certificate as varchar(7)) + '; ' + cast(train.year as varchar(4)) + ' г.' as [Тип транспорта, сертификат,
                    год постройки],price as [Страховая стоимость, BYN], sostav.summa as [Страховая сумма, BYN],cast( case when pay.vidpay = 'Наличные' 
                    then coefficient*0.24 * 1 else coefficient * 0.9 end as decimal(18, 2)) as [Корректировочный коэффициент, %], (0.24) as 
                    [Базовый коэффициент, %],cast( case when pay.vidpay = 'Наличные' then coefficient*0.24 * 1 else coefficient * 0.24 * 0.9 end as 
                    decimal(18, 2)) as [Страховой тариф, %],cast( case when pay.vidpay = 'Наличные' then sostav.summa * (coefficient * 0.24) * 1 else 
                    sostav.summa * (coefficient * 0.24) * 0.9 end as decimal(18, 2))  as [Страховая премия, BYN] from train, sostav, correctionfactor, pay
                    where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} and sostav.idtreaty = pay.idtreaty and coefficient in ( select TOP(Select count(
                    sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} )((case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')
                    then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '>=') then (case when((case when(SUBSTRING(note, 0, 3)= '>=' or 
                    SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)<= (Select count(sostav.idsostav) 
                    from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} )) then coefficient else 0 end)else (case when((case when(SUBSTRING(note, 0, 3)= 
                    '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<=') then (case when((case when(SUBSTRING
                    (note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)>= (Select 
                    count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} )) then coefficient else 0 end)else 0 end) end))from train, sostav, correctionfactor
                    where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} group by idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer,price,
                    sostav.summa ,sostav.idsostav,note,coefficient order by correctionfactor.note Desc) group by idsostav,sostav.idtreaty, sostav.idtrain,
                    train.name ,train.nomer,price, sostav.summa ,sostav.idsostav,note,coefficient,vidpay,type,certificate,year having((case when((case when(
                    SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '>=') then (case when(
                    (case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) 
                    end)<= (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} )) then coefficient else 0 end)else (case when((case when(SUBSTRING(note, 0, 3)= 
                    '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<=') then (case when((case when(
                    SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)>= 
                    (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 0 end)else 0 end) end)) != 0 union Select train.name + '; ' 
                    + cast(train.nomer as varchar(50)) as [Название транспорта, номер],train.type + '; ' + cast(train.certificate as varchar(7)) + '; ' +
                    cast(train.year as varchar(4)) + ' г.' as [Тип транспорта, сертификат, год постройки],price as [Страховая стоимость, BYN], sostav.summa 
                    as [Страховая сумма, BYN],cast( case when pay.vidpay = 'Наличные' then coefficient*0.24 * 1 else coefficient * 0.9 end as decimal(18, 2)) 
                    as [Корректировочный коэффициент, %], (0.24) as [Базовый коэффициент, %],cast( case when pay.vidpay = 'Наличные' then coefficient*0.24 * 1
                    else coefficient * 0.24 * 0.9 end as decimal(18, 2)) as [Страховой тариф, %],cast( case when pay.vidpay = 'Наличные' then sostav.summa * 
                    (coefficient * 0.24) * 1 else sostav.summa * (coefficient * 0.24) * 0.9 end as decimal(18, 2))  as [Страховая премия, BYN] from train, 
                    sostav, correctionfactor, pay where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} and sostav.idtreaty = pay.idtreaty and coefficient in (select TOP(Select 
                    count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} )((case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')
                    then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '=') then (case when((case when(SUBSTRING(note, 0, 3)= '>=' or 
                    SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)= (Select count(sostav.idsostav) 
                    ...
                    (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 0 end)else 0 end) end)) from train, sostav, correctionfactor
                    where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' group by idsostav,sostav.idtreaty, sostav.idtrain,
                    train.name ,train.nomer, price, sostav.summa ,sostav.idsostav,note,coefficient order by correctionfactor.note Desc) group by idsostav,
                    sostav.idtreaty, sostav.idtrain, train.name ,train.nomer, price, sostav.summa ,sostav.idsostav,note,coefficient,vidpay,type,certificate,
                    year having((case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else 
                    SUBSTRING(note, 0, 2) end)= '=') then (case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then 
                    SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)= (Select count(sostav.idsostav) from sostav inner join treaty on 
                    sostav.idtreaty = treaty.idtreaty where sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} )) then coefficient else
                    0 end)else (case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else 
                    SUBSTRING(note, 0, 2) end)= '>') then (case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then 
                    SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)< (Select count(sostav.idsostav) from sostav inner join treaty on 
                    sostav.idtreaty = treaty.idtreaty where sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 0 
                    end)else 0 end) end))!= 0 union Select train.name + '; ' + cast(train.nomer as varchar(50)) as [Название транспорта, номер],
                    train.type + '; ' + cast(train.certificate as varchar(7)) + '; ' +cast(train.year as varchar(4)) + ' г.' as [Тип транспорта, сертификат,
                    год постройки],price as [Страховая стоимость, BYN], sostav.summa as [Страховая сумма, BYN],cast( case when pay.vidpay = 'Наличные' then 
                    coefficient*0.24 * 1 else coefficient * 0.9 end as decimal(18, 2)) as [Корректировочный коэффициент, %], (0.24) as 
                    [Базовый коэффициент, %], cast( case when pay.vidpay = 'Наличные' then coefficient*0.24 * 1 else coefficient * 0.24 * 0.9 end as 
                    decimal(18, 2)) as [Страховой тариф, %],cast( case when pay.vidpay = 'Наличные' then sostav.summa * (coefficient * 0.24) * 1 else 
                    sostav.summa * (coefficient * 0.24) * 0.9 end as decimal(18, 2))  as [Страховая премия, BYN] from train, sostav, correctionfactor, pay
                    where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty = pay.idtreaty and sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} and coefficient in (select TOP(Select count(sostav.idsostav) from sostav inner join
                    treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} )((case when(
                    (case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<') then
                    (case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note))else 
                    SUBSTRING(note, 2, len(note)) end)> (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty 
                    where sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 0 end)else (case when((case when
                    (SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<=') then
                    (case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note))else 
                    SUBSTRING(note, 2, len(note)) end)>= (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty 
                    where sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 0 end)else 0 end) end)) from train,
                    sostav, correctionfactor where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty = 75 group by
                    idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer,price, sostav.summa ,sostav.idsostav,note,coefficient order by 
                    correctionfactor.note Desc) group by idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer,price, sostav.summa ,
                    sostav.idsostav,note,coefficient,vidpay,certificate,type,year having((case when((case when(SUBSTRING(note, 0, 3)= '>=' or 
                    SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<') then (case when((case when(SUBSTRING(note, 0, 3)
                    = '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)> (Select 
                    count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 0 end)else (case when((case when(SUBSTRING(note, 0, 3)= '>='
                    or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<=') then (case when((case when(
                    SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) end)>= 
                    ...
                    cast(train.year as varchar(4)) + ' г.' as [Тип транспорта, сертификат, год постройки],price as [Страховая стоимость, BYN], sostav.summa 
                    as [Страховая сумма, BYN],cast( case when pay.vidpay = 'Наличные' then coefficient*0.24 * 1 else coefficient * 0.9 end as decimal(18, 2))
                    as [Корректировочный коэффициент, %], (0.24) as [Базовый коэффициент, %],cast( case when pay.vidpay = 'Наличные' then coefficient*0.24 * 
                    1 else coefficient * 0.24 * 0.9 end as decimal(18, 2)) as [Страховой тариф, %],cast( case when pay.vidpay = 'Наличные' then sostav.summa
                    * (coefficient * 0.24) * 1 else sostav.summa * (coefficient * 0.24) * 0.9 end as decimal(18, 2))  as [Страховая премия, BYN] from train,
                    sostav, correctionfactor, pay where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty = 
                    pay.idtreaty and sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} group by idsostav,sostav.idtreaty, sostav.idtrain,
                    train.name ,train.nomer,price, sostav.summa ,sostav.idsostav,note,coefficient,vidpay,certificate,year,type having(select TOP(1)((case when
                    ((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<') then
                    (case when((case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else 
                    SUBSTRING(note, 2, len(note)) end)> (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty 
                    where sostav.idtreaty ={dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 1 end)else (case when((case when(
                    SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,0,3) else SUBSTRING(note, 0, 2) end)= '<=') then (case when(
                    (case when(SUBSTRING(note, 0, 3)= '>=' or SUBSTRING(note,0,3)= '<=')then SUBSTRING(note,3,len(note)) else SUBSTRING(note, 2, len(note)) 
                    end)>= (Select count(sostav.idsostav) from sostav inner join treaty on sostav.idtreaty = treaty.idtreaty where sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} ))then coefficient else 1 end)else 1 end) end))from train, sostav, correctionfactor
                    where train.idtrain = sostav.idtrain and correctionfactor.name like '%средств%' and sostav.idtreaty =
                    {dataGridView1.CurrentRow.Cells[0].Value.ToString()} group by idsostav,sostav.idtreaty, sostav.idtrain, train.name ,train.nomer,price, 
                    sostav.summa ,sostav.idsostav,note,coefficient order by correctionfactor.note Desc)= 1", sqlConnection);
                    command.Fill(dataSet3);
                    dataGridView2.DataSource = dataSet3.Tables[0].DefaultView;
                    dataGridView2.AllowUserToAddRows = false;
                    dataGridView2.RowsDefaultCellStyle.WrapMode = DataGridViewTriState.True;
                    ...
                    if (dataGridView2.RowCount > 0)
                    {
                        ...
                        try
                        {
                            if (dataGridView2.Rows.Count > 0)
                            {
                                for (int i = 0; i < dataGridView2.RowCount; i++)
                                {
                                    sumid = sumid + Convert.ToDecimal(dataGridView2[3, i].Value.ToString());
                                    textBox13.Text = Convert.ToString(Convert.ToDecimal(textBox13.Text) + Convert.ToDecimal(dataGridView2[7, i].Value.ToString()));
                                }
                            }
                        }
                        catch { textBox13.Text = "0"; }
