...
namespace Agent.Form
{
    public partial class Pays : UserControl
    {
        ...
        public void Pays_load2()
        {
            sqlConnection.Open();
            DataSet dataSet = new DataSet();
            SqlDataAdapter command = new SqlDataAdapter($@"Select idpay,pay.idtreaty as [Номер договора] ,datepay as [Дата оплаты],summa as [Сумма взноса],vidpay as [Вид оплаты] from pay inner join treaty on pay.idtreaty=treaty.idtreaty inner join bid on treaty.idbid=bid.idbid where bid.idpolicyholder={policyholder.idakk}", sqlConnection);
            command.Fill(dataSet);
            dataGridView1.DataSource = dataSet.Tables[0].DefaultView;
            sqlConnection.Close();
            dataGridView1.Columns[0].Visible = false;
...
where treaty.term!='Единовременно' and bid.idpolicyholder={policyholder.idakk}
group by pay.idtreaty,treaty.term,treaty.idtreaty
having (treaty.term='В два срока' and 2-count(pay.idtreaty)!=0 and treaty.term!='Ежеквартально') 
or
(4-count(pay.idtreaty)!=0 and treaty.term='Ежеквартально')";
        ...
        {
            panel3.Visible = false;
            button3.Image = new Bitmap(@"/Diplom/proga/Agent/Agent/Resources/pngwing.com (48).png");
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                button3.Image = new Bitmap(@"/Diplom/proga/Agent/Agent/Resources/pngwing.com (47).png");
            }
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            try
            {
                if (label6.Text == "Добавить оплату")
                {
                    ...
                    sqlConnection.Open();
                    SqlCommand command = new SqlCommand($@"INSERT INTO [pay](idtreaty,datepay,summa,vidpay)
                    VALUES (@idt,@date,@s,@v);", sqlConnection);
